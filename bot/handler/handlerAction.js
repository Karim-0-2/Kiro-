...
module.exports = (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) => {
  ...
  return async function (event) {
    ...
    // ✅ Restrict usage to only Owner or Bot Admins
    if (event.senderID !== OWNER_UID && !global.config.ADMINBOT.includes(event.senderID)) {
      return api.sendMessage(
        "❌ Only the bot owner or bot admins can use this bot.",
        event.threadID,
        event.messageID
      );
    }
      ...
      case "message_reaction":
        ...
        if (["😾", "🤬", "😡", "😠"].includes(event.reaction)) {
          if (event.senderID == api.getCurrentUserID()) {
            if (["61575011217788", "100083520680035"].includes(event.userID)) {
