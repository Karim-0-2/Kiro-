...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "Kiss someone you tag or reply to",
    longDescription: "Generates a kiss image with you and the person you tag or reply to",
    category: "love",
  ...
  onStart: async function ({ api, event }) {
    ...
    // Get the target user: reply > mention
    let target;
    if (messageReply && messageReply.senderID) {
      ...
      return api.sendMessage("❌ Please tag someone or reply to their message!", threadID, messageID);
    }
    ...
    try {
      const imgPath = await makeImage({ one: senderID, two: target });
