...
module.exports = {
  config: {
    ...
    shortDescription: "Manage all friend requests (admins only)",
    longDescription: "Accept or reject all friend requests in one page",
    category: "admin",
    guide: {
      en: "{pn} → view friend requests\n{pn} add <numbers|all> → accept\n{pn} del <numbers|all> → reject"
    }
  },
  ...
  onReply: async function ({ Reply, event, api }) {
    ...
    if (args[0] === "add") {
      ...
      return api.sendMessage("❌ Invalid command. Use: add <num|all> / del <num|all>", event.threadID, event.messageID);
    }

    let targetIDs = args.slice(1);
    if (args[1] === "all") {
      targetIDs = Array.from({ length: Reply.listRequest.length }, (_, i) => i + 1);
    }
    ...
    for (const stt of targetIDs) {
      const user = Reply.listRequest[parseInt(stt) - 1];
      if (!user) {
    ...
    if (replyMsg) {
      ...
      api.sendMessage("❌ No valid requests were processed.", event.threadID, event.messageID);
    }
    ...
    try {
      const form = {
        ...
        variables: JSON.stringify({
          input: { scale: 3, first: 500 } // fetch up to 500 requests in one page
        })
      };
      ...
      console.error(error);
      api.sendMessage("❌ Error while fetching friend requests.", event.threadID);
    }
  }
};
