...
config: {
    ...
    author: "Hasib",
    role: 0,
    shortDescription: "VIP system with expiration, warnings, admin ban & Super VIP",
    category: "admin"
},
...
langs: {
    en: {
        ...
        superVipAdded: "✅ Super VIP add (%1) by Hasib",
        superVipRemoved: "❌ Super VIP removed (%1) by Hasib",
        superVipNoPermission: "⚠️ Only Hasib can use Super VIP commands!"
    }
},
...
onStart: async function ({ message, args, event, role, api }) {
    ...
    if (args[0] === "list") {  
        ...
        for (const u of data) {  
            ...
            if (remaining > 0) {  
                ...
                listMsg += `• (${name}) – ${formatTime(remaining)}${superVips.includes(u.uid) ? " ⭐" : ""}\n`;  
            }  
        }  
    ...
    if (args[0] === "supervip") {
        ...
        const uid = event.messageReply?.senderID;
        if (!uid) return message.reply("⚠️ Reply to a user to manage Super VIP.");
        ...
        if (args[1] === "remove") {
            ...
            const userInfo = await api.getUserInfo(uid);
            const name = userInfo[uid]?.name || uid;
            const msg = await message.reply(module.exports.langs.en.superVipRemoved.replace("%1", name));
            setTimeout(() => api.unsendMessage(msg.messageID).catch(() => {}), 5000);
        }
    }
}
