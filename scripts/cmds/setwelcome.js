...
module.exports = {
        ...
        guide: {
            vi: {
                body: "   {pn} text [<nội dung> | reset]: chỉnh sửa nội dung văn bản hoặc reset về mặc định, với những shortcut có sẵn:"
                    ...
                    + "\n\n   Reply hoặc gửi kèm một tin nhắn có file với nội dung {pn} file: để thêm tệp đính kèm vào tin nhắn chào mừng (ảnh, video, audio, GIF)"
            },
            en: {
                body: "   {pn} text [<content> | reset]: edit text content or reset to default, with some shortcuts:"
                    ...
                    + "\n\n   Reply or send a message with file with content {pn} file: to add attachments (image, video, audio, GIF)"
            }
        }
    },
    ...
    langs: {
        vi: {
            ...
            missingFile: "Hãy phản hồi tin nhắn này kèm file ảnh/video/audio/GIF",
        ...
        en: {
            ...
            missingFile: "Please reply this message with image/video/audio/GIF file",
        ...
        switch (args[0]) {
            ...
            case "file": {
                if (args[1] == "reset") {
                    ...
                    if (!welcomeAttachment) return message.reply(getLang("noFile"));
                    try {
...
async function saveChanges(message, event, threadID, senderID, threadsData, getLang) {
    ...
    const attachments = [...event.attachments, ...(event.messageReply?.attachments || [])]
        .filter(item => ["photo", "png", "animated_image", "video", "audio"].includes(item.type) || (item.url && item.url.endsWith(".gif")));
    ...
    await Promise.all(attachments.map(async attachment => {
        ...
        const ext = getExtFromUrl(url) || "gif";
