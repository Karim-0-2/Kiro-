...
module.exports.run = async ({ api, args, event, message }) => {
  ...
  if (urlYtb) {
    ...
    return api.sendMessage(
      { body: title, attachment: await downloadFile(downloadLink, 'audio.mp3') },
      event.threadID,
  ...
  if (!result.length) return api.sendMessage(`⭕ No search results match the keyword: ${keyWord}`, event.threadID, event.messageID);
  ...
  api.sendMessage(
    { body: msg + "Reply to this message with a number you want to listen", attachment: await Promise.all(thumbnails) },
    event.threadID,
    (err, info) => {
  ...
  try {
    ...
    if (!isNaN(choice) && choice > 0 && choice <= result.length) {
      ...
      await api.sendMessage(
        { body: `• Title: ${title}\n• Quality: ${quality}`, attachment: await downloadFile(downloadLink, 'audio.mp3') },
        event.threadID,
        () => fs.unlinkSync('audio.mp3'),
        event.messageID
      );
