...
// Fixed Owner (always allowed)
const OWNER_UID = "61557991443492"; // Hasib

// Ensure VIP & SuperVIP files exist
...
module.exports = {
  ...
  onStart: async function ({ message, event, role }) {
    ...
    const superVips = JSON.parse(fs.readFileSync(superVipPath));

    const isVip = vips.some(u => u.uid === event.senderID);
    const isSuperVip = superVips.includes(event.senderID);
    const isOwner = event.senderID === OWNER_UID;

    // Restrict usage
    if (!isOwner && (isVip || role >= 1) && !isSuperVip) {
      return message.reply("⛔ Only Super VIP ⭐ or Owner can use this command!");
    }
